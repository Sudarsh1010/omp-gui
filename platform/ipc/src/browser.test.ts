    ...
    it("lands Takeover's dispatched Input.dispatch* on the live page (click + typed text)", async () => {
      ...
      try {
        ...
        try {
          ...
          expect(
            asString(
              await evaluate(cdp, targetSessionId, "btn.dataset.clicked"),
              "btn.dataset.clicked",
            ),
          ).toBe("yes");
          ...
          expect(asString(await evaluate(cdp, targetSessionId, "field.value"), "field.value")).toBe(
            "hi",
          );
        } finally {
          cdp.close();
...
async function stopRelay(child: ChildProcessByStdio<null, Readable, Readable>): Promise<void> {
  child.kill();
  await once(child, "exit");
}
