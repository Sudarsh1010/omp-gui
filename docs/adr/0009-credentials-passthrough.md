...
Logout is the one deviation from "the app renders login UX only when omp emits it": rpc-ui has no `logout` command (only `login`), so `auth_logout` shells out directly to `omp auth-broker logout <id>` via the same scratch-cwd CLI helper (`omp_cli.rs`'s `run_omp_cli`) the Settings config bridge uses, rather than riding a running session. This is safe because logout is a direct credential-store mutation with no OAuth round trip and no session-specific state to coordinate — unlike login, which stays on the rpc-ui pass-through described above.
